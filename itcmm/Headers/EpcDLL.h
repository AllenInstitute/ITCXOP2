/*
   ...
   ##################################################################
   #####  Copyright 1990, 2012 by HEKA Elektronik               #####
   #####  All rights reserved                                   #####
   ##################################################################
...
enum  // EPC9_ElectrodeModeType
{
   EPC9_Two_Electrodes        = 0,
   EPC9_Three_Electrodes      = 1
};

// Definitions for EPC9_SetCFastMode
enum  // EPC9_CFastModeType
{
   EPC9_Cfast_Lookup          = 0,
   EPC9_Cfast_Computed        = 1
};
...
// LIH_OptionsType
// Record used to pass options to the 3 initialization functions:
//    LIH_InitializeInterface
//    EPC9_InitializeAndCheckForLife
//    PG300_InitializeAndCheckForLife
typedef struct
{
   EPC_INT32 UseUSB;
...
/*
   EPC9_InitializeAndCheckForLife
      ...
      - The pointer "PathToScaleFile" can be NUL or point to the file path
        ...
        This parameter is not required for the EPC10_USB, since it does not use
        external scale files.
      - The pointer "OptionsPtr" can be NUL or point to a struct containing
        ...
        -- OptionsPtr->FIFOSamples
               for LIH1600 and LIH8+8: maximal size of virtual FIFO buffer
        ...
        -- OptionsPtr->ExternalScaling
               for LIH8+8: if OptionsPtr->ExternalScaling = EPC_TRUE then raw DAC 
      ...
      Some options and functions are specific to particular amplifier version.
      There are 2 possibilities to detect, whether a particular option is supported
      by the active ampifier:
      1. Set the option, e.g. EPC9_SetVmonX100, and read back, whether the setting
         has been set.
		2. The function EPC9_HasProperty reports, whether the inquired option is
         supported by the chosen amplifier.
*/
...
// EPC9_SetAutoSettings
// Sets the parameters usend in the "auto" functions:
// CSlowPeak
...
//    response while performing an auto cslow compensation.
// CSlowTimeout
//    Sets the number of seconds after which AutoCSlow will abort.
// RsCompTimeout
//    Sets the number of seconds after which AutoRsComp will abort.
// CFastTimeout
//    Sets the number of seconds after which AutoCFast will abort.
// VpOffsetTimeout
//    Sets the number of seconds after which AutoVpOffset and AutoSearch will abort.
// GLeakTimeout
//    Sets the number of seconds after which AutoGleak will abort.
void EPC_Calling EPC9_SetAutoSettings(
            EPC_LONGREAL   CSlowPeak,
            EPC_INT32      CSlowCycles,
            EPC_LONGREAL   CSlowTimeout,
            EPC_LONGREAL   RsCompTimeout,
            EPC_LONGREAL   CFastTimeout,
            EPC_LONGREAL   VpOffsetTimeout,
            EPC_LONGREAL   GLeakTimeout ) EPC_Import;

void EPC_Calling EPC9_GetAutoSettings(
            EPC_LONGREAL   *CSlowPeak,
            EPC_INT32      *CSlowCycles,
            EPC_LONGREAL   *CSlowTimeout,
            EPC_LONGREAL   *RsCompTimeout,
...
// EPC9_SetRsCorrection
// Sets the fraction and correction values separately, if Prediction = true.
void EPC_Calling EPC9_SetRsCorrection(
            EPC_LONGREAL Fraction,
            EPC_LONGREAL Correction,
            EPC_BOOLEAN Prediction ) EPC_Import;

// EPC9_AutoRsComp
// Performs AutoRs compensation. Please consult
...
// EPC9_SetF1Bandwidth
// Sets that Filter1 setting which is nearest to the given bandwidth.
// Please note that Filter1 should never be set when in current clamp
// mode, or when RsCompensation is active.
...
// EPC9_SetF2Response
// Sets the Filter2 response characteristics, definition of
// EPC9_F2ResponseType see above.
void EPC_Calling EPC9_SetF2Response( EPC_INT32 F2Response ) EPC_Import;

// EPC9_SetElectrodeMode
// Defines the 3 electrode input, definition of
// EPC9_ElectrodeModeType see above.
void EPC_Calling EPC9_SetElectrodeMode( EPC_INT32 ElectrodeMode ) EPC_Import;
...
// EPC9_SetCFastMode
// Sets the algorith used for AutoCFast., definition of
// EPC9_CFastModeType see above.
void EPC_Calling EPC9_SetCFastMode( EPC_INT32 CFastMode ) EPC_Import;

// EPC9_SetCFastHigh
// The extended CFast range is set, if TRUE is passed. Can only be set for
// EPC10 with the property "EPC9_HasCFastHigh".
// Maximal C-fast value is about:
//    15.9pF in the normal C-fast range
//    200pF in the extended C-fast range
void EPC_Calling EPC9_SetCFastHigh( EPC_BOOLEAN CFastHighOn ) EPC_Import;
   
// EPC9_SetVrefX2
// The extended stimulus range is set, if TRUE is passed. Can only be set for
// EPC10 with the property "EPC9_HasVrefX2AndF2Vmon".
// Returns TRUE, if no error occurred when loading the appropriate scale file,
// since there are 2 scale files, one for each stimulus range.
// Maximal stimulus range is :
//    +/- 1V in the normal stimulus range
//    +/- 5V in the extended C-stimulus range
EPC_BOOLEAN EPC_Calling EPC9_SetVrefX2( EPC_BOOLEAN VrefX2On ) EPC_Import;
...
void EPC_Calling EPC9_GetRsCorrection(
            EPC_LONGREAL *Fraction,
            EPC_LONGREAL *Correction,
            EPC_BOOLEAN *Prediction ) EPC_Import;
...
EPC_INT32 EPC_Calling EPC9_GetF2Response( void ) EPC_Import;

EPC_INT32 EPC_Calling EPC9_GetElectrodeMode( void ) EPC_Import;
...
EPC_INT32 EPC_Calling EPC9_GetCFastMode( void ) EPC_Import;

EPC_BOOLEAN EPC_Calling EPC9_GetCFastHigh( void ) EPC_Import;

EPC_BOOLEAN EPC_Calling EPC9_GetVrefX2( void ) EPC_Import;
...
// LIH_Close
// Closes the connection to the hardware to allow another application to
// connect to the hardware. Only connection to hardware is closed, hardware
// is not affected.
void EPC_Calling LIH_Close( void ) EPC_Import;

// LIH_Open
// Opens the connection to the hardware after connection was closed by LIH_Close.
// Hardware must have been initialized before calling LIH_Close or LIH_Open
// by a call to LIH_InitializeInterface or EPC9_InitializeAndCheckForLife.
// Otherwise, hardware and drivers may be in an undefined state.
EPC_BOOLEAN EPC_Calling LIH_Open( void ) EPC_Import;
...
// Deprecated function.
// The following functions should not be used anylongerThere are more appropriate ones.

// EPC9_SetF2Butterworth
// Sets the Filter2 current response characteristics. True sets
...
// The definitions of amplifiers for the function
//    PG300_InitializeAndCheckForLife
enum  // PG300_AmplifierType
{
   PG300_PG300Ampl               = 0,
   PG300_PG300USBAmpl            = 1
};

// Values returned by PG300_InitializeAndCheckForLife
// PG300_Success is returned upon success, i.e., no error occurred.
// PG300_NoScaleFile is returned, when the P3SCALE file was not found with
// the give file path.
// PG300_LIHInitFailed is returned, when the AD-board could not be
// initialized. Other error number may be returned. Errors are returned
// in the error message string in human readable form.
enum  // PG300_ResultType 
{
   PG300_Success                 = 0,
   PG300_LIHInitFailed           = 1,
   PG300_FirmwareError           = 2,
   PG300_NoScaleFile             = 3
};


// Definitions for PG300_SetActiveAmpl
enum  // PG300_ActiveAmplType
{
   PG300_Disk                    = 0,
   PG300_Ring                    = 1
};

// Definitions for PG300_SetCurrentRange
enum  // PG300_SetCurrentRangeType
{
   PG300_Range2A                 = 0,
   PG300_Range1A                 = 1,
   PG300_Range100mA              = 2,
...
// Definitions for PG300_SetCurrentFilterResponse
enum  // PG300_CurrentFilterResponseType
{
   PG300_Bessel                  = 0,
   PG300_Butterworth             = 1,
   PG300_Bypassed                = 2
...
// Definitions for PG300_SetControlBandwidth
enum  // PG300_ControlBandwidthType
{
   PG300_CtrlBandwidth100Hz      = 0,
   PG300_CtrlBandwidth300Hz      = 1,
   PG300_CtrlBandwidth1kHz       = 2,
   PG300_CtrlBandwidth3kHz       = 3,
   PG300_CtrlBandwidth10kHz      = 4,
   PG300_CtrlBandwidth30kHz      = 5,
   PG300_CtrlBandwidth100kHz     = 6,
   PG300_CtrlBandwidth300kHz     = 7,
   PG300_CtrlBandwidthMax        = 8,
   PG300_CtrlBandwidth10Hz       = 9
};

// Definitions for PG300_SetExternalPreAmplifierCurrentBandwidth
enum  // PG300_PreAmpCurrentBandwidthType
{
   PG300_PreAmpCurrBandw10kHz    = 0,
   PG300_PreAmpCurrBandw30kHz    = 1,
   PG300_PreAmpCurrBandw30kHzHQ  = 2,
   PG300_PreAmpCurrBandw100kHz   = 3
};

// Definitions for PG300_SetVoltageBandwidth
enum  // PG300_VoltageBandwidthType
{
   PG300_VoltageBandwidth10Hz    = 0,
   PG300_VoltageBandwidth100Hz   = 1,
   PG300_VoltageBandwidth1kHz    = 2,
   PG300_VoltageBandwidthMax     = 3
};

// Definitions for PG300_SetStimFilter
enum  // PG300_StimFilterType
{
   PG300_StimFilter_500kHz       = 0,
   PG300_StimFilter_50kHz        = 1,
   PG300_StimFilter_5kHz         = 2,
   PG300_StimFilter_500Hz        = 3,
   PG300_StimFilter_Bypass       = 4,
   PG300_StimFilter_10kHz_new    = 5,
   PG300_StimFilter_7kHz_new     = 6,
   PG300_StimFilter_5kHz_new     = 7,
   PG300_StimFilter_3kHz_new     = 8,
   PG300_StimFilter_2kHz_new     = 9,
   PG300_StimFilter_1kHz_new     = 10
};

// Definitions for PG300_SetExternalInput
enum  // PG300_ExternalInputType
{
   PG300_ExtInputOff             = 0,
   PG300_ExtInputD10             = 1,
   PG300_ExtInputOn              = 2
...
// Definitions for PG300_SetAmplMode
enum  // PG300_AmplModeType
{
   PG300_PotentiostaticMode      = 0,
   PG300_GalvanostaticMode       = 1
};

// Definitions for PG300_SetElectrodeMode
enum  // PG300_ElectrodeModeType
{
   PG300_Mode3Electrodes         = 0,
   PG300_Mode4Electrodes         = 1
};

// Definitions for PG300_SetCellMode
enum  // PG300_CellModeType
{
   PG300_CellMode                = 0,
   PG300_StandbyMode             = 1,
   PG300_OCPMode                 = 2
...
// Definitions for PG300_SetVoltageRange
enum  // PG300_VoltageRangeType
{
   PG300_Volt_10V                = 0,
   PG300_Volt_5V                 = 1,
   PG300_Volt_2V                 = 2
...
// Definitions for PG300_HasProperty
enum  // PG300_PropertyType
{
   PG300_Has90V                  = 0,
   PG300_HasRingAmpifier         = 1,
   PG300_Has10HzCtrlBandw        = 2,
   PG300_Has300kHzCtrlBandw      = 3,
   PG300_HasCurrF1Max            = 4,
   PG300_HasLargeEEPROM          = 5,
   PG300_HasNewStimFilter        = 6,
   PG300_CanHaveExtPreAmp        = 7
};


/*
   PG300_InitializeAndCheckForLife
      - Initializes the amplifier, e.g., PG300 or PG300_USB
         "Amplifier" defines which amplifier is to be used:
            PG300_PG300Ampl
            PG300_PG300USBAmpl
      - Returns the value "PG300_Success", if the initialization succeeded;
        otherwise returns an error code, in which case the error description
        is returned in the "ErrorMessage" string.
      - The pointer "ErrorMessage" can be NUL or point to a string at least
        256 characters long. "ErrorMessage" will contain the message describing
        the result of the initialization in case an error occurred.
      - The pointer "PathToScaleFile" can be NUL or point to the file path
        of the SCALE.PG3 file. The path separator is "\" for Windows
        and "/" for MacOS (":" can be used for MacOS, when "System 9" style
        paths are to be used, e.g., when called from inside IGOR Pro).
        The path string must be end with the path separator.
        This parameter is not required for the PG300_USB, since it does not use
        external scale files.
      - The pointer "OptionsPtr" can be NUL or point to a struct containing
        the options to set for the acquisition board, and to get from
        the amplifier. Make sure to initialize all fields, best by setting the
        ...
        -- OptionsPtr->BoardNumber
               PCI slot index:
                  ...
                  LIH8+8:  see OptionsPtr->DeviceNumber
        -- OptionsPtr->FIFOSamples
               for LIH8+8: maximal size of virtual FIFO buffer
        -- OptionsPtr->DeviceNumber
               for LIH8+8:
                  if empty and OptionsPtr->BoardNumber = 0: uses first device found,
                  if empty and OptionsPtr->BoardNumber > 0: uses nth device
      ...
      There are 2 possibilities to detect, whether a particular option is supported
      by the active ampifier:
      1. Set the option, e.g. PG300_StimFilter_10kHz_new, and read back, whether
         the setting has been set.
      2. The function PG300_HasProperty reports, whether the inquired option is
         supported by the chosen amplifier.
*/
EPC_INT32 EPC_Calling PG300_InitializeAndCheckForLife(
            EPC_Str256Ptr ErrorMessage, 
            EPC_INT32 Amplifier, 
            EPC_Str256Ptr PathToScaleFile,
            LIH_OptionsPtr OptionsPtr,
...
// and the given amplifiers connected to rach1 and rack2.
// This function is to be called, when the amplifier is off-line.
// It allows the function PG300_HasProperty to return correct properties,
// when the amplifier is off-line.
void EPC_Calling PG300_ForceVersion( EPC_CHAR Version ) EPC_Import;

// PG300_HasProperty
// Returns true, if the inquired property is supported by the active amplifier.
// Requires that the amplifier is properly initialized. If the amplifier is
// off-line, the function PG300_ForceVersion had to be issued, passing the
// amplifier version to be used.
// Definition of PG300_PropertyType see above.
EPC_BOOLEAN EPC_Calling PG300_HasProperty( EPC_INT32 Property ) EPC_Import;

EPC_LONGREAL EPC_Calling PG300_GetEcell( EPC_INT32 Samples ) EPC_Import;

EPC_LONGREAL EPC_Calling PG300_GetIcell( EPC_INT32 Samples ) EPC_Import;

void EPC_Calling PG300_SetEInit( EPC_LONGREAL Potential ) EPC_Import;

void EPC_Calling PG300_SetIInit( EPC_LONGREAL Current ) EPC_Import;
   
// PG300_Shutdown
// Sets the potentiostat in a save mode, and sets all
// DAC outputs to zero, then terminates connections to drivers.
void EPC_Calling PG300_Shutdown( void ) EPC_Import;

// PG300_FlushCache
// Commands sent to the PG300 are not immediately transmitted.
// The reason is that transmission takes time depending on bus type.
// To speed-up over-all performance, commands are cached and transmitted either when:
// 1. An acquisition command is issued to the interface, e.g. LIH_ReadAll or
//    LIH_StartStimAndSample. Or
// 2. PG300_FlushCache is called.
void EPC_Calling PG300_FlushCache( void ) EPC_Import;

// PG300_FlushThenWait
// Flushes the command cache (see PG300_FlushCache) then waits the given number of seconds.
void EPC_Calling PG300_FlushThenWait( EPC_LONGREAL Seconds ) EPC_Import;

// PG300_SetCurrentRange
// Sets the current range of the amplifiers,
// definition of PG300_CurrentRangeType see above.
void EPC_Calling PG300_SetCurrentRange( EPC_INT32 CurrentRange ) EPC_Import;

EPC_INT32 EPC_Calling PG300_GetCurrentRange( void ) EPC_Import;

// PG300_SetControlBandwidth
// Sets the control bandwidth of the amplifiers,
// definition of PG300_ControlBandwidthType see above.
void EPC_Calling PG300_SetControlBandwidth( EPC_INT32 ControlBandwidth ) EPC_Import;

EPC_INT32 EPC_Calling PG300_GetControlBandwidth( void ) EPC_Import;

// PG300_SetVoltageBandwidth
// Sets the bandwidth of the voltage trace,
// definition of PG300_VoltageBandwidthType see above.
void EPC_Calling PG300_SetVoltageBandwidth( EPC_INT32 VoltageBandwidth ) EPC_Import;

EPC_INT32 EPC_Calling PG300_GetVoltageBandwidth( void ) EPC_Import;

// PG300_SetStimFilter
// Sets the stimulus filter bandwith,
// definition of PG300_StimFilterType see above.
void EPC_Calling PG300_SetStimFilter( EPC_INT32 StimFilter ) EPC_Import;

EPC_INT32 EPC_Calling PG300_GetStimFilter( void ) EPC_Import;

// PG300_SetRemote
// Sets the amplifier to remode mode, or turns it off.
void EPC_Calling PG300_SetRemote( EPC_BOOLEAN On ) EPC_Import;

EPC_BOOLEAN EPC_Calling PG300_GetRemote( void ) EPC_Import;

// PG300_SetExternalInput
// Sets the external input mode,
// definition of PG300_ExternalInputType see above.
void EPC_Calling PG300_SetExternalInput( EPC_INT32 ExtInput ) EPC_Import;

EPC_INT32 EPC_Calling PG300_GetExternalInput( void ) EPC_Import;

// PG300_SetCellMode
// Sets the amplifier cell mode.
// Returns TRUE, when the cell mode could be set. Cell mode and OCP mode cannot
// be set, if the manual stanby botton is activated.
// Definition of PG300_CellModeType see above.
EPC_BOOLEAN EPC_Calling PG300_SetCellMode( EPC_INT32 CellMode ) EPC_Import;

EPC_INT32 EPC_Calling PG300_GetCellMode( void ) EPC_Import;

// PG300_SetAmplMode
// Sets potentiostatic or galvanostatic mode,
// definition of PG300_AmplModeType see above.
void EPC_Calling PG300_SetAmplMode( EPC_INT32 AmplMode ) EPC_Import;

EPC_INT32 EPC_Calling PG300_GetAmplMode( void ) EPC_Import;

// PG300_SetElectrodeMode
// Sets 3 or 4 electrode mode,
// definition of PG300_ElectrodeModeType see above.
void EPC_Calling PG300_SetElectrodeMode( EPC_INT32 Mode ) EPC_Import;

EPC_INT32 EPC_Calling PG300_GetElectrodeMode( void ) EPC_Import;

// PG300_SetCurrentBandwidth
// Sets the bandwidth in [Hz] of the current trace.
void EPC_Calling PG300_SetCurrentBandwidth( EPC_LONGREAL Hz ) EPC_Import;

EPC_LONGREAL EPC_Calling PG300_GetCurrentBandwidth( void ) EPC_Import;

// PG300_SetCurrentFilterResponse
// Sets response type ot the current filter,
// definition of PG300_CurrentFilterResponseType see above.
void EPC_Calling PG300_SetCurrentFilterResponse( EPC_INT32 Response ) EPC_Import;

EPC_INT32 EPC_Calling PG300_GetCurrentFilterResponse( void ) EPC_Import;

// PG300_SetIRCompensationOn
// Activates IR-compensation or turns it off.
void EPC_Calling PG300_SetIRCompensationOn( EPC_BOOLEAN On ) EPC_Import;

EPC_BOOLEAN EPC_Calling PG300_GetIRCompensationOn( void ) EPC_Import;

// PG300_SetIRCompensation
// Sets the value in [Ohm] of the IR-compensation.
void EPC_Calling PG300_SetIRCompensation( EPC_LONGREAL Ohm ) EPC_Import;

EPC_LONGREAL EPC_Calling PG300_GetIRCompensation( void ) EPC_Import;

// PG300_SetVoltageNotchFilter
// Activates the voltage notch filter or turns it off.
void EPC_Calling PG300_SetVoltageNotchFilter( EPC_BOOLEAN On ) EPC_Import;

EPC_BOOLEAN EPC_Calling PG300_GetVoltageNotchFilter( void ) EPC_Import;

// PG300_SetVoltageNotchFilter
// Activates the curent notch filter or turns it off.
void EPC_Calling PG300_SetCurrentNotchFilter( EPC_BOOLEAN On ) EPC_Import;

EPC_BOOLEAN EPC_Calling PG300_GetCurrentNotchFilter( void ) EPC_Import;

// PG300_SetActiveAmpl
// PG340 only: Selects the disk or ring amplifier as the active one,
// i.e. the amplifer to which commands are sent and from which parameters
// are obtained.
// Definition of PG300_ActiveAmplType see above.
void EPC_Calling PG300_SetActiveAmpl( EPC_INT32 NewAmpl ) EPC_Import;

EPC_INT32 EPC_Calling PG300_GetActiveAmpl( void ) EPC_Import;

// PG300_SetOffline
// Sets the on-line or off-line.
void EPC_Calling PG300_SetOffline( EPC_BOOLEAN Offline ) EPC_Import;

EPC_BOOLEAN EPC_Calling PG300_GetOffline( void ) EPC_Import;

// PG300_SetVoltageRange
// Sets voltage output and input range,
// definition of PG300_VoltageRangeType see above.
void EPC_Calling PG300_SetVoltageRange( EPC_INT32 Factor ) EPC_Import;

EPC_INT32 EPC_Calling PG300_GetVoltageRange( void ) EPC_Import;

// PG300_GetRealCurrentGain
// Returns the active gain of the amplifier in Ohm, i.e. V/A.
EPC_LONGREAL EPC_Calling PG300_GetRealCurrentGain( EPC_INT32 CurrentRange ) EPC_Import;

// PG300_GetOverload
// Returns TRUE, if the amplifier is in overload.
EPC_BOOLEAN EPC_Calling PG300_GetOverload( void ) EPC_Import;

// PG300_GetManualStandby
// Returns TRUE, if the standby button is pressed.
EPC_BOOLEAN EPC_Calling PG300_GetManualStandby( void ) EPC_Import;

// PG300_GetExternalPreAmplifierPresent
// Returns TRUE, if the external pre-amplifer probe is connected.
EPC_BOOLEAN EPC_Calling PG300_GetExternalPreAmplifierPresent( void ) EPC_Import;

// PG300_SetExternalPreAmplifier
// Activates the external pre-amplifer probe or turns it off.
void EPC_Calling PG300_SetExternalPreAmplifier( EPC_BOOLEAN On ) EPC_Import;

EPC_BOOLEAN EPC_Calling PG300_GetExternalPreAmplifier( void ) EPC_Import;

// PG300_SetExternalPreAmplifierCurrentBandwidth
// Sets the bandwidth of the pre-amplifier current trace.
// definition of PG300_PreAmpCurrentBandwidthType see above.
void EPC_Calling PG300_SetExternalPreAmplifierCurrentBandwidth( EPC_INT32 Bandwidth ) EPC_Import;

EPC_INT32 EPC_Calling PG300_GetExternalPreAmplifierCurrentBandwidth( void ) EPC_Import;
